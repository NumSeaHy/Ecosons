...
function bathymetryCrosses_octave(baths::Vector{Bathymetry}, dp::Int64)
    TRANSECTCROSS = TransectCross()
    ...
    for dsn in 1:length(baths)
        bath = baths[dsn]
        for n in 1:dp:(length(bath.time) - dp)
            p1 = [bath.latitude[n], bath.longitude[n]]
            p2 = [bath.latitude[n + dp], bath.longitude[n + dp]]
            p1p = [p1[2], -p1[1]]
            p2p = [p2[2], -p2[1]]

            for dsm in (dsn + 1):length(baths)
                bat2 = baths[dsm]
                for m in 1:dp:(length(bat2.time) - dp)
                    q1 = [bat2.latitude[m], bat2.longitude[m]]
                    q2 = [bat2.latitude[m + dp], bat2.longitude[m + dp]]
                    q1p = [q1[2], -q1[1]]
                    q2p = [q2[2], -q2[1]]

                    rb = dot(p2 .- p1, q2p .- q1p)
                    sb = dot(q2 .- q1, p2p .- p1p)

                    if rb != 0 && sb != 0
                        ra = dot(q1 .- p1, q2p .- q1p)
                        sa = dot(p1 .- q1, p2p .- p1p)
                        ...
                        if 0 < r < 1 && 0 < s < 1
                            Ccount += 1
                            push!(C1lat, (1 - r) * bath.latitude[n] + r * bath.latitude[n + dp])
                            push!(C1lon, (1 - r) * bath.longitude[n] + r * bath.longitude[n + dp])
                            push!(C2lat, (1 - s) * bat2.latitude[m] + s * bat2.latitude[m + dp])
                            push!(C2lon, (1 - s) * bat2.longitude[m] + s * bat2.longitude[m + dp])
                            ...
                            depth1 = (1 - r) * bath.depth[n] + r * bath.depth[n + dp]
                            depth2 = (1 - s) * bat2.depth[m] + s * bat2.depth[m + dp]
                            push!(C12dz, depth1 - depth2)
                        end
                    end
                end
            end
